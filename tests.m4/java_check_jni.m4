...
if test "$have_ar" = "yes"; then
 if test "$PHP_JAVA" != "yes"; then
  ...
  LD_LIBRARY_PATH=`pwd` $PHP_JAVA/bin/java -Djava.library.path=`pwd` jni 2>>test.log && jb_cv_have_jni=yes 
 else 
  $CC $lt_prog_compiler_pic -olibjni.so -shared jni.c 2>test.log && jb_cv_have_jni=yes
 fi
fi
